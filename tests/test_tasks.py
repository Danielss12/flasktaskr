...
class TasksTests(unittest.TestCase):
	...
	def test_users_cannot_delete_tasks_that_are_not_created_by_them(self):
		...
		self.create_task()
		self.logout()
		self.create_user('jose', 'jose@realpython.com', 'python101')
		self.login('jose', 'python101')
		self.app.get('tasks/', follow_redirects=True)
		response= self.app.get('delete/1/', follow_redirects=True)
	...
	def test_tasks_template_displays_logged_in_user_name(self):
		self.register('Fletcher', 'fletcher@me.com', 'python101', 'python101')
		self.login('Fletcher', 'python101')
		response = self.app.get('tasks/', follow_redirects=True)
		self.assertIn(b'Fletcher', response.data)

	def test_users_cant_see_task_modify_links_if_not_their_own(self):
		self.register('eu', 'eu@eu.com', 'python', 'python')
		self.login('eu', 'python')
		self.app.get('tasks/', follow_redirects=True)
		self.create_task()
		self.logout()
		self.register('eu2', 'eu2@eu.com', 'python2', 'python2')
		response = self.login('eu2', 'python2')
		self.app.get('tasks/', follow_redirects=True)
